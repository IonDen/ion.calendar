        ...
        init: function (options) {
            ...
            return this.each(function () {
                ...
                var prepareCalendar = function () {
                    ...
                    // Fetch events
                    settings.eventSource(moment('01.' + timeNowLocal.format('MM.YYYY'), 'DD.MM.YYYY'), moment(monthLastDay + '.' + timeNowLocal.format('MM.YYYY'), 'DD.MM.YYYY'), function (events) {
                        ...
                        if (settings.sundayFirst) {
                            ...
                            for (i = 1; i <= monthLastDay; i++) {
                                ...
                                // Get the data of the day's event
                                var event_data = {};
                                if (typeof events[i] !== 'undefined') {
                                    if (typeof events[i].date !== 'undefined') {
                                        event_data = events[i];
                                    }
                                }

                                // current day
                                if (moment(timeNowLocal).date(i).format("DD.MM.YYYY") === timeNow.format("DD.MM.YYYY")) {
                                    html += '<td class="ic__day ic__day_state_current ' + ((event_data.style) ? event_data.style : '') + '">' + i + '</td>';
                                } else if (timeSelected && moment(timeNowLocal).date(i).format("DD.MM.YYYY") === timeSelected.format("DD.MM.YYYY")) {
                                    html += '<td class="ic__day ic__day_state_selected ' + ((event_data.style) ? event_data.style : '') + '">' + i + '</td>';
                                } else {
                                    html += '<td class="ic__day ' + ((event_data.style) ? event_data.style : '') + '">' + i + '</td>';
                                }
